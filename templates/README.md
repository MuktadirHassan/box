...
UID/GID, shell, prompt, template revision, and insecure-mode state as build
arguments. The catalog port separates consumers from this embedded adapter, so
future local or downloaded providers can be registered without UI or backend
changes.

The default `ubuntu-24.04-terminal-tools` template builds a writable development
image with baseline tools and passwordless in-container sudo. It includes Podman
only when insecure mode explicitly delegates the host rootless Podman API. The
runtime stays rootless and starts as the configured non-root user.
